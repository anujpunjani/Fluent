...
class Interpreter {
	...
	visit_VarAccessNode(node, context) {
		...
		if (!value)
			return response.failure(
				new RuntimeError(node.positionStart, node.positionEnd, `'${varName}' is not defined`, context)
			);
	...
	visit_VarReAssignNode(node, context) {
		let response = new RuntimeResult();
		let VarAccessNode = response.register(this.visit(node.VarAccessNode, context));
		if (response.shouldReturn()) return response;
		let varName = node.VarAccessNode.varNameToken.value;
		let value = response.register(this.visit(node.newValueNode, context));
		if (response.shouldReturn()) return response;
	...
	visit_NumberNode(node, context) {
		return new RuntimeResult().success(
			new Number(node.token.value).setContext(context).setPosition(node.positionStart, node.positionEnd)
		);
	}
	...
	visit_StringNode(node, context) {
		return new RuntimeResult().success(
			new String(node.token.value).setContext(context).setPosition(node.positionStart, node.positionEnd)
		);
	}
	...
	visit_ForNode(node, context) {
		...
		return response.success(
			node.shouldReturnNull ? Number.null : new List(elements).setPosition(node.positionStart, node.positionEnd)
		);
	}
	...
	visit_WhileNode(node, context) {
		...
		while (true) {
			let condition = response.register(this.visit(node.conditionNode, context));
			if (response.shouldReturn()) return response;
	...
	visit_FuncDefNode(node, context) {
		...
		let funcValue = new Function(funcName, bodyNode, argNames, node.shouldAuoReturn)
			.setContext(context)
