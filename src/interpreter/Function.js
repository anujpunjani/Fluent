...
class Function extends BaseFunction {
	...
	execute(interpreter, args) {
		...
		response.register(this.checkAndPopulateArgs(this.argNames, args, executionContext));
		if (response.shouldReturn()) return response;

		let value = response.register(interpreter.visit(this.bodyNode, executionContext));
		if (response.shouldReturn() && response.functionReturnValue == null) return response;
	...
	copy() {
		let copy = new Function(this.functionName, this.bodyNode, this.argNames, this.shouldAutoReturn);
