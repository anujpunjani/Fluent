...
class BuiltInFunction extends BaseFunction {
	...
	execute(interpreter, args) {
		...
		response.register(this.checkAndPopulateArgs(this[methodArgs], args, executionContext));
	...
	execute_isNumber(executionContext) {
		let value = executionContext.symbolTable.get("value");
		return new RuntimeResult().success(value.name == "Number" ? Number.true : Number.false);
	}
	...
	execute_isString(executionContext) {
		let value = executionContext.symbolTable.get("value");
		return new RuntimeResult().success(value.name == "String" ? Number.true : Number.false);
	}
	/**
	 * @param {Context} executionContext
	 */
	execute_isList(executionContext) {
		let value = executionContext.symbolTable.get("value");
		return new RuntimeResult().success(value.name == "List" ? Number.true : Number.false);
	}
	...
	execute_isFunction(executionContext) {
		let value = executionContext.symbolTable.get("value");
		return new RuntimeResult().success(value instanceof BaseFunction ? Number.true : Number.false);
	}
	...
	execute_remove(executionContext) {
		...
		if (index.value < 0 || index.value >= list.elements.length)
			return new RuntimeResult().failure(
				new RuntimeError(this.positionStart, this.positionEnd, `Index out of range`, executionContext)
			);
		...
		try {
			...
			return new RuntimeResult().failure(
				new RuntimeError(this.positionStart, this.positionEnd, e.message, executionContext)
			);
		}
	...
	execute_len(executionContext) {
		...
		if (list.name != "List")
			return new RuntimeResult().failure(
				new RuntimeError(this.positionStart, this.positionEnd, `Argument must be a list`, executionContext)
			);

		return new RuntimeResult().success(new Number(list.elements.length));
	}
