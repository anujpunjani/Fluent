...
class Parser {
	...
	statements() {
		...
		return response.success(
			statements.length == 1
				? statements[0]
				: new ListNode(statements, positionStart, this.currentToken.positionEnd.copy())
		);
	}
	...
	statement() {
		...
		if (this.currentToken.matches(TokenTypes.KEYWORD, "return")) {
			...
			return response.success(new ReturnNode(expr, positionStart, this.currentToken.positionStart.copy()));
		}

		if (this.currentToken.matches(TokenTypes.KEYWORD, "continue")) {
			response.registerAdvancement();
			this.advance();
			return response.success(new ContinueNode(positionStart, this.currentToken.positionStart.copy()));
		}

		if (this.currentToken.matches(TokenTypes.KEYWORD, "break")) {
			response.registerAdvancement();
			this.advance();
			return response.success(new BreakNode(positionStart, this.currentToken.positionStart.copy()));
		}

		if (this.currentToken.matches(TokenTypes.KEYWORD, "pass")) {
			response.registerAdvancement();
			this.advance();
			return response.success(new PassNode(positionStart, this.currentToken.positionStart.copy()));
		}
		...
		if (response.error) {
			return response.failure(
				new InvalidSyntaxError(
					this.currentToken.positionStart,
					this.currentToken.positionEnd,
					"Expected 'return', 'continue', 'break', 'pass', 'let', 'if', 'for', 'while', 'function', 'define',  int, float, identifier, '+', '-', '(', '[' or 'not'"
				)
			);
		}
	...
	forExpr() {
		...
		if (!this.currentToken.matches(TokenTypes.KEYWORD, "for")) {
			return response.failure(
				new InvalidSyntaxError(this.currentToken.positionStart, this.currentToken.positionEnd, "Expected 'for'")
			);
		}
		...
		return response.success(new ForNode(varName, startValue, endValue, stepValue, body, true));
		// }
	 ...
	 */
	power() {
		return this.binaryOperation(this.call.bind(this), [TokenTypes.POW, TokenTypes.MOD], this.factor.bind(this));
	}
	...
	atom() {
		...
		if ([TokenTypes.INT, TokenTypes.FLOAT].includes(this.currentToken.type)) {
			...
			if (this.currentToken.type == TokenTypes.EQ) {
				...
				let expr = response.register(this.expr());
				if (response.error) return response;
				return response.success(new VarReAssignNode(new VarAccessNode(token), expr));
			}
			...
			this.currentToken.matches(TokenTypes.KEYWORD, "define") ||
			this.currentToken.matches(TokenTypes.KEYWORD, "function")
		) {
		...
		return response.failure(
			new InvalidSyntaxError(
				this.currentToken.positionStart,
				this.currentToken.positionEnd,
				"Expected int, float, string, identifier, '+', '-', '(', '[', 'if', 'for', 'while', 'function' or 'define'"
			)
		);
	}
	...
	listExpr() {
		...
		return response.success(new ListNode(elementNodes, positionStart, this.currentToken.positionEnd.copy()));
	}
	 ...
	 */
	term() {
		return this.binaryOperation(this.factor.bind(this), [TokenTypes.MUL, TokenTypes.DIV]);
	}

	arithExpr() {
		return this.binaryOperation(this.term.bind(this), [TokenTypes.PLUS, TokenTypes.MINUS]);
	}
	...
	expr() {
		...
		if (this.currentToken.matches(TokenTypes.KEYWORD, "let")) {
			response.registerAdvancement();
			this.advance();
		...
		let node = response.register(
			this.binaryOperation(this.compExpr.bind(this), [TokenTypes.KEYWORD + "and", TokenTypes.KEYWORD + "or"])
		);
		if (response.error)
			return response.failure(
				new InvalidSyntaxError(
					this.currentToken.positionStart,
					this.currentToken.positionEnd,
					"Expected 'let', 'if', 'for', 'while', 'function', 'define',  int, float, identifier, '+', '-', '(', '[' or 'not'"
				)
			);
		return response.success(node);
	}
	...
	funcDef() {
		...
		if (this.currentToken.type == TokenTypes.ARROW) {
			...
			return response.success(new FuncDefNode(funcName, argNames, NodeToReturn, true));
		}
		response.register(this.continueIfNewLine());
