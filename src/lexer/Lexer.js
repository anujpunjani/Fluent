...
const { illegalCharError } = require("../errors");
...
class Lexer {
	...
	generateTokens() {
		...
		while (this.currentChar != null) {
			if (" \t\r".includes(this.currentChar)) {
				...
				this.advance();
				return { tokens: null, error: new illegalCharError(positionStart, this.position, `'${char}'`) };
			}
		}
	...
	makeNotEquals() {
		...
		if (this.currentChar == "=") {
			this.advance();
			return { token: new Token(TokenTypes.NE, null, positionStart, this.position), error: null };
		}

		this.advance();
		return { token: null, error: new ExpectedCharError(positionStart, this.position, "'=' (after '!')") };
	}
	...
	makeNumber() {
		...
		if (dotCount === 0) return new Token("INT", parseInt(num), positionStart, this.position);
		else return new Token("FLOAT", parseFloat(num), positionStart, this.position);
	}
	...
	makeString() {
		...
		while (this.currentChar != null && (this.currentChar != '"' || escapeCharacter)) {
			if (escapeCharacter) {
				string += escapeCharacters.get(this.currentChar, this.currentChar);
